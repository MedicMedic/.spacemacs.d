* Mac Os Mojave + DigitalOcean + Shadowsocks VPS搭建&优化 
之前一直用那些免流的付费vpn翻墙，又慢又不稳定（留下了又懒又蠢的泪水），刚好学了《计算机网络》，就想这个自己来配置一个vps，又快又稳定，参考了[[http://daipei.me/posts/cross_firewall_with_digitalocean_shadowsocks/]]，虽然碰到了很多困难，但是最后还是成功了。
下面我就写一下我的具体操作步骤，以及遇到的一些问题。
** 注册并租用VPS
** 配置VPS
*** 远程登陆服务器
打开终端输入以下代码(我用的是ipv4的地址，ipv6和private ip好像都不dei行）
#+begin_src shell
$ ssh root@[YOUR VPS IP ADDRESS]
#+end_src
*** 小麻烦
按道理以上代码应该能够顺利连接上远程服务器，但是就在我高兴的想要进行下一步的时候，我的ITerm上就出现红色的大❌
#+begin_src shell
/Users/anthony/.ssh/config line 3: garbage at end of line; "-%r@%h:%p".
#+end_src
what?这个“-%r@%h:%p”是个什么东西，也太鬼畜了吧，我打开~/.ssh/config赶紧看了一眼
#+begin_src shell
Host *
ControlMaster auto
ControlPath ~/.ssh/master -%r@%h:%p
ControlPersist = no%
#+end_src
网上找了一圈没有任何解答，那咋办吗，直接删掉233
#+begin_src shell
ControlPath ~/.ssh/master -%r@%h:%p
#+end_src
������
#+begin_src 
ControlPath ~/.ssh/master 
#+end_src
成功连接！
*** 配置防火墙 
**** 1.安装firewalld 
#+begin_src shell
$ yum install firewalld firewall-config
#+end_src
**** 2.启动firewalld
#+begin_src shell
$ systemctl start firewalld
#+end_src
**** #修改SSH端口
#+begin_src shell
$ vi /usr/lib/firewalld/services/ssh.xml
#+end_src
出现以下窗口：
#+begin_src xml
<?xml version="1.0" encoding="utf-8"?>
　　<service>
　　　　<short>SSH</short>
   　　<description>Secure Shell (SSH) is a protocol for logging into and executing commands on remote machines. It provides secure encrypted communications. If you plan on accessing your machine remotely via SSH over a firewalled interface， enable this option. You need the openssh-server package installed for this option to be useful.</description>
   　　<port protocol="tcp" port="22"/>
　　</service>
#+end_src
如果想要修改，将 port="22" 改为 port="xxx" （建议不要修改）
修改后重载firewalld
#+begin_src shell
$ firewall-cmd --permanent --add-service=ssh
$ firewall-cmd --reload
#+end_src
执行上述命令如果显示FirewallD is not running，使用 systemctl start firewalld 启动防火墙。

注意如果在搭建完成后无法科学上网，很有可能是防火墙导致的，快速定位问题可以使用以下命令关闭防火墙测试：
#+begin_src shell
$ systemctl stop firewalld
#+end_src
如果关闭防火墙后可以正常科学上网，说明是端口没有正确添加到防火墙中。
*** 安装shadowsocks
**** 1.安装pip
#+begin_src shell
$ yum install m2crypto python-setuptools
$ easy_install pip
#+end_src
**** 2.使用pip安装shadowsocks
#+begin_src shell
$ pip install shadowsocks
#+end_src
**** 3.创建配置文件
在/etc目录下添加文件夹shadowsocks
#+begin_src shell
$ mkdir /etc/shadowsocks
#+end_src
创建配置文件shadowsocks.json
#+begin_src shell
$  vi /etc/shadowsocks/shadowsocks.json
#+end_src
按i进入vim的insert模式粘贴以下代码
单端口单账户
#+begin_src json
{
    "server":"server_ip",
    "server_port":8388,
    "local_address": "127.0.0.1",
    "local_port":1080,
    "password":"yourPassword",
    "timeout":300,
    "method":"aes-256-cfb",
    "fast_open": false
}
#+end_src
多端口多账户
#+begin_src json
{
 "server":"yourServerIp",
 "_comment": {
 "8380":"me",
 "8381": "girl",
 "8382": "godness"
},
 "local_address":"127.0.0.1",
 "local_port":1080,
 "port_password":{
 "8380":"pc9527",
 "8381":"pc9527",
 "8382":"pc9527"
},
 "timeput":300,
 "method":"aes-256-cfb",
 "fast_open":false
}
#+end_src
此处method为加密方式（method），我在有些配置教程中看到有推荐使用rc4-md5，但是官方不推荐使用，因为安全度较低
以下还有几个不推荐加密方式
| Name     | Key Size | IV Length |
|----------+----------+-----------|
| bf-cfb   |       16 |         8 |
|----------+----------+-----------|
| chacha20 |       32 |         8 |
|----------+----------+-----------|
| salsa20  |       32 |         8 |
|----------+----------+-----------|
| rc4-md5  |       16 |        16 |
|----------+----------+-----------|
其他配置以及加密方式可参考改文档[[https://wiki.archlinux.org/index.php/Shadowsocks_(简体中文)#配置代理
]]启动
#+begin_src shell
$ sudo ssserver -c /etc/shadowsocks/shadowsocks.json -d start
#+end_src
停止
#+begin_src shell
$ ssserver -c /etc/shadowsocks/shadowsocks.json -d stop
#+end_src
启动配置文件
#+begin_src shell
$ ssserver -c /etc/shadowsocks/shadowsocks.json --user nobody -d start
#+end_src
*** 打开防火墙
查看默认防火墙状态（关闭后显示notrunning，开启后显示running）
#+begin_src shell
$ firewall-cmd --state
#+end_src
启动firewall
#+begin_src shell
systemctl start firewalld.service
#+end_src
开放xxx端口(多端口多账户情况请循环调用♻️）
#+begin_src shell
firewall-cmd --zone=public --add-port=8381/tcp --permanent
#+end_src
重启firewall
#+begin_src shell
firewall-cmd --reload
#+end_src
查看已经开放的端口
#+begin_src shell
firewall-cmd --list-ports
#+end_src
*** 设置shadowsocks开机自启
**** 1.安装supervisor
#+begin_src shell
$ easy_install supervisor
#+end_src
**** 2.创建配置文件
#+begin_src shell
$ echo_supervisord_conf > /etc/supervisord.conf
#+end_src
**** 3.修改配置文件
#+begin_src shell
$ vi /etc/supervisord.conf
#+end_src
**** 4.在文件末尾加上：
#+begin_src conf
[program:ssserver]
command = ssserver -c /etc/shadowsocks.json
autostart=true
autorestart=true
#+end_src
**** 5.设置supervisor开机自启
#+begin_src shell
$ vi /etc/rc.local
#+end_src
在文件末尾另起一行，添加： supervisord
**** 6.CentOS7 还需要为 rc.local 添加执行权限
#+begin_src shell
$ chmod 777 /etc/rc.local
#+end_src
这样ss开启自动后台运行就配置好了，下面重启服务器就好了，可以在digitalocean控制面板中重新启动服务器。
*** 优化
**** 内核参数优化
调整内核参数
修改配置文件 /etc/sysctl.conf
#+begin_src confg
fs.file-max = 51200

net.core.rmem_max = 67108864
net.core.wmem_max = 67108864
net.core.netdev_max_backlog = 250000
net.core.somaxconn = 4096

net.ipv4.tcp_syncookies = 1
net.ipv4.tcp_tw_reuse = 1
net.ipv4.tcp_tw_recycle = 0
net.ipv4.tcp_fin_timeout = 30
net.ipv4.tcp_keepalive_time = 1200
net.ipv4.ip_local_port_range = 10000 65000
net.ipv4.tcp_max_syn_backlog = 8192
net.ipv4.tcp_max_tw_buckets = 5000
net.ipv4.tcp_fastopen = 3
net.ipv4.tcp_rmem = 4096 87380 67108864
net.ipv4.tcp_wmem = 4096 65536 67108864
net.ipv4.tcp_mtu_probing = 1
net.ipv4.tcp_congestion_control = hybla
#+end_src
运行sysctl -p生效
#+begin_src shell
$ sysctl -p
#+end_src
**** 锐速加速
由于本人缺乏Linux方面的知识并且对速度没有太高的要求，因此暂时留坑，不过可以参考以下教程，我觉得讲的还是挺清楚
[[https://www.cmsky.com/serverspeeder-install/]]
[[https://www.cnblogs.com/semonxv/p/3816366.html
]]


*** 其他参考文章
[[https://www.jianshu.com/p/e990897aeaec]]
[[http://wuchong.me/blog/2015/02/02/shadowsocks-install-and-optimize/]]
